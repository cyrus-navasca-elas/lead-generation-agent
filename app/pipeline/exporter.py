...
def _write_companies_sheet(ws, scored: list[ScoredCompany]) -> None:
    ...
    for row_idx, item in enumerate(scored, start=2):
        ...
        score_cell = ws.cell(row=row_idx, column=14, value=item.total_score)
        ws.cell(row=row_idx, column=15, value=s.confidence if s else "")
        ws.cell(row=row_idx, column=16, value="\n".join(s.pain_points) if s else "")
        ws.cell(row=row_idx, column=17, value=s.fit_reason if s else "")
        ws.cell(row=row_idx, column=18, value=s.erp_relevance if s else "")
    ...
    _wrap_text_columns(ws, [16, 17, 18])
